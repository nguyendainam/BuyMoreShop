...
export default function Header() {
  ...
  const OptionCategory = () => {
    const dataCategory = categoryData.map(item => ({
      ...
      label: item.title,
      key: item.key,
      children: item.list.length > 0 ? (
        item.list?.map(i => (
          {
            label: i.name,
            key: i.key,
            children: i.listItem.length > 0 ? (
              i.listItem.map((i1) => ({
                label: i1.name,
                key: i1.keyItem
              }))
    ...
    const categoryMenuItems = (
      ...
      >
        {dataCategory.map(item => (
          <>
            <Menu.SubMenu key={item.key} title={item.label} icon={BiSolidCartAlt} >
              <div className={style.formListItem}>
                {item.children && item.children.map(child => (
                  <div className={style.formCategory}>
                    <div className={style.titleCat} key={child.key}>{child.label}</div>
                    {item.children && item.children.map(itemchild => (
                      <div className={style.itemCat} key={itemchild.key} onClick={() => alert(itemchild.key)}>
                        {itemchild.label}
                      </div>
                    ))}
                  </div>
